...
def database_pgsql_url_config():
    return str(setting().DB_CONNECTION + "://" + setting().DB_USERNAME + 
    ":" + setting().DB_PASSWORD + "@" + setting().DB_HOST + ":" + 
    setting().DB_PORT + "/" + setting().DB_DATABASE)


# # 2. using starlette to load .env configuration file
# def database_pgsql_url_config():
#     conf = Config(".env")
#     return str(conf("DB_CONNECTION") + "://" + conf("DB_USERNAME") + 
